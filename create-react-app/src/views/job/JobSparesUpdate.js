...
const JobSparesUpdate = ({ data, updateData }) => {
  ...
  const addAdditionalRows = () => {
    const newRows = [...Array(1)].map(() => ({ sparesAndLabour: '', qty: '', rate: '', amount: '' }));
  ...
  const handleRowDelete = (rowIndex) => {
    ...
    if (newRows.length > 0) {
      ...
      updateData([...Array(1)].map(() => ({ sparesAndLabour: '', qty: '', rate: '', amount: '' })));
    }
  };
        ...
        <Grid container direction="row" spacing={gridSpacing}>
          <Grid item xs={12}>
            <Table>
              <TableHead>
                <TableRow>
                  <TableCell>Spares & Labour</TableCell>
              ...
              <TableBody>
                {data.map((row, index) => (
                  <TableRow key={index}>
                    ...
                    <TableCell>
                      <TextField fullWidth value={row?.qty || ''} onChange={(e) => handleInputChange(index, 'qty', e.target.value)} />
                    </TableCell>
                    <TableCell>
